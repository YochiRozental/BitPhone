...
//         if (res.success) {
//             onLoginSuccess({ phone: form.phone, idNum: form.idNum, secret: form.secret });
...
export default function AuthForm({ onLoginSuccess }: AuthFormProps) {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if (res.success) {
            // נחלץ את השם מהתשובה אם יש, אחרת נשתמש בשם ריק
            const loggedUser: User = {
                phone: form.phone,
                idNum: form.idNum,
                secret: form.secret,
                name: res.data?.name || "", // ✅ מאובטח ובלי שגיאת טיפוס
            };
